      ...
      <header className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-8 mb-16 mt-4">
        <div className="relative group">
          <div className="w-32 h-32 md:w-48 md:h-48 rounded-full border-[4px] border-black overflow-hidden shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-pink-300">
            <img
              src={settings?.profile_url || "https://via.placeholder.com/150"}
              className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700"
            />
          </div>
          <Heart
            className="absolute -bottom-1 -right-1 bg-yellow-300 border-2 border-black p-2 rounded-full animate-pulse"
            size={40}
            fill="red"
          />
        </div>
        ...
        <div className="flex-1 text-center md:text-left">
          <h1 className="text-[12vw] md:text-[80px] font-black italic uppercase tracking-tighter leading-[0.8] mb-4 text-black">
            flyhighlana.
          </h1>
          <div className="h-1 w-full bg-black mb-2" />
          <p className="text-[10px] font-mono uppercase tracking-[0.5em] opacity-50 italic">
            Pinned Memories & Digital Journal
          </p>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-24">
        <AnimatePresence>
          {pinnedQuotes.map((q, i) => (
            ...
            >
              {q.image_url && (
                <div className="h-48 border-b-[3px] border-black overflow-hidden">
                  <img
                    src={q.image_url}
              ...
              <div className="p-6">
                ...
                <p className="text-xl font-bold italic leading-tight mb-4">
                  "{q.text}"
                </p>
      ...
      <div className="fixed bottom-8 right-8 z-50 flex items-center gap-4">
        {isPlaying && (
          <motion.div
            ...
            className="hidden md:block bg-white border-2 border-black px-4 py-2 font-mono text-[10px] font-bold uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] tracking-widest"
          >
            NOW PLAYING: {settings?.music_title || "LANA RADIO"}
          </motion.div>
        )}
        <audio ref={audioRef} src={settings?.music_url} loop />
        <button
          onClick={toggleMusic}
          className="w-16 h-16 rounded-full bg-black text-white border-4 border-white shadow-[6px_6px_0px_0px_rgba(0,0,0,0.3)] flex items-center justify-center hover:scale-110 active:scale-95 transition-all"
        >
  ...
  return (
    <div className="min-h-screen bg-[#E9E4D9] p-6 md:p-12 font-serif">
      <div className="max-w-6xl mx-auto">
        <Link
          to="/"
          className="inline-flex items-center gap-2 mb-10 font-black uppercase text-xs border-b-2 border-black pb-1 tracking-widest"
        >
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-16">
          <input
            ...
            className="flex-1 p-4 border-[3px] border-black bg-white outline-none font-mono text-sm shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
            onChange={(e) => setSearch(e.target.value)}
          />
          <select
            className="p-4 border-[3px] border-black bg-white font-mono text-sm shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {filtered.map(q => (
            <div
              key={q.id}
              className="bg-white border-[3px] border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] overflow-hidden flex flex-col"
            >
              {q.image_url && (
                <img
                  src={q.image_url}
                  className="w-full h-56 object-cover border-b-2 border-black"
                />
              )}
              <div className="p-6">
